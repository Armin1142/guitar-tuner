...
10. Sağ üstte 5 renkli tema seçici (kahverengi/varsayılan, siyah, beyaz, mavi, yeşil) — `style.css`'in başındaki `:root[data-theme="..."]` blokları CSS değişkenlerini üzerine yazıyor. Seçim `localStorage`'da (`gitar-akort-theme`) hatırlanıyor. Akort durum renkleri (yeşil/sarı/kırmızı) anlamsal oldukları için kasıtlı olarak temadan bağımsız/sabit tutuldu.
...
12. Tema seçici baştan eksikti — sadece arkaplan/vurgu rengi değişiyordu, metinlerin çoğu (başlık, ipucu yazıları, tel düğmeleri, Hz metni vb.) sabit renkte kalıyordu, bu yüzden özellikle "beyaz" gibi açık bir tema seçildiğinde bazı yazılar okunmuyordu. Düzeltme iki parçalıydı: (1) `style.css`'teki hemen hemen tüm sabit renkler `--text-1/2/3` (birincil/ikincil/soluk metin) ve `--surface`/`--surface-hover` (nota rozeti, tel düğmeleri, geçmiş grafiği kutusu gibi iç yüzeyler) değişkenlerine taşındı; (2) `script.js` içinde akort durumu boştayken (`cents == null`) JS'in nota rengini/ibre rengini/durum balonu rengini hep aynı sabit hex'e zorlaması engellendi — bunun yerine inline stil boşaltılıp CSS'teki tema değişkeninin devreye girmesine izin veriliyor.
13. Tema sistemi ikinci ve daha kapsamlı bir geçirmeden geçti: **artık tema değiştirildiğinde arayüzün tamamı seçilen renge uyum sağlıyor.** Bu sefer taranan/düzeltilen yerler: cent göstergesinin dış halkası ve ibresi (SVG `stroke`/`fill` öznitelikleri artık `var(--border)`/`var(--text-3)`), gitar kafasının (headstock) ahşap gövdesi ve tel çizgileri (`#woodGrad` gradyanı artık `var(--surface-hover)` → `var(--bg-2)`, çerçeve `var(--text-3)`), klavye ile gezinirken görünen focus halkaları (`:focus-visible` artık `var(--accent)` kullanıyor). **Tek istisna:** burgu başları (tuning pegs) — gerçekçi metal görünüm için kasıtlı olarak sabit gri/gümüş bırakıldı, gitarın geri kalanı (gövde, teller) artık temaya göre renk değiştiriyor. Ayrıca `script.js`'teki artık kullanılmayan `NEUTRAL`/`IDLE_NOTE_COLOR` sabitleri temizlendi ve tema değişkenlerinin başına, gelecekte yeni tema eklemeyi anlatan bir yönerge yorumu eklendi.
14. Tellerin gitar kafasından sonra nereye bağlandığı belirsizdi — önceki illüstrasyonda teller SVG'nin alt kenarında aniden kesiliyordu. Şimdi teller görünür bir "köprü" (bridge) şekline kadar uzatıldı ve SVG'nin `viewBox`'ı buna yer açacak şekilde büyütüldü (yükseklik 420 → 460).
15. Referans sesi: başlıkta bir 🔊/🔇 butonu, açıkken seçili tele tıklandığında o telin hedef frekansında (`STRINGS[i].freq`) kısa bir sinüs tonu çalıyor (Web Audio `OscillatorNode` + `GainNode`, ~1.3sn, tık sesi olmasın diye yumuşak giriş/çıkış zarfı ile). Mikrofonun kendi `AudioContext`'inden tamamen ayrı, biri kapalıyken diğeri çalışabiliyor. Buton da diğer her şey gibi tema değişkenleriyle boyanıyor ve aktifken `var(--accent)` rengini alıyor; tooltip metni 10 dilin hepsinde çevrildi.

## Dosyalar

- `index.html` — sayfa yapısı, SVG gitar kafası illüstrasyonu
- `style.css` — 5 renkli tema (kahverengi/siyah/beyaz/mavi/yeşil) ve tüm görsel stiller
